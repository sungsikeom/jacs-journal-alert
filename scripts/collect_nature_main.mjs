...
const OUTPUT = path.join(ROOT, "data", "nature_articles.json");
const PROFILE = process.env.NATURE_CHROME_PROFILE_DIR
  ? path.resolve(process.env.NATURE_CHROME_PROFILE_DIR)
  : path.join(os.tmpdir(), "journal-pulse-nature-main-profile-v2");
const BASE_URL = "https://www.nature.com/nature/research-articles";
const SCOPE_START = "2026-01-01";
const DOI_PREFIX = "10.1038/s41586-";
const PAGE_SIZE = 20;
const MINIMUM_BASELINE = 500;
const forceBaseline = process.argv.includes("--fresh");

function normalizeDoi(value) {
  const article = String(value || "").match(/\/articles\/(s41586-[^/?#]+)/i);
  if (article) return `10.1038/${article[1].toLowerCase()}`;
  const doi = String(value || "").toLowerCase().match(/10\.1038\/s41586-[^?#\s]+/i);
...
async function readRows(page) {
  return page.locator("li.app-article-list-row__item").evaluateAll((items) => items.map((item) => {
    const anchors = [...item.querySelectorAll('a[href*="/articles/s41586-"]')];
    const anchor = anchors.find((candidate) => /\/articles\/s41586-/i.test(candidate.href || ""));
...
async function stablePageRows(page, expectedCount) {
  ...
  throw new Error(`Nature page did not fully load: expected ${expectedCount}, found ${best.length}`);
}
...
async function run() {
  ...
  try {
    ...
    console.log(`Opening Nature 2026 research articles in ${process.env.NATURE_HEADLESS === "0" ? "visible" : "headless"} Chrome`);
    ...
    if (!Number.isInteger(officialTotal) || officialTotal < MINIMUM_BASELINE) {
      throw new Error(`Nature 2026 total count was not found or is implausible: ${officialTotal || "unknown"}`);
    }
    ...
    for (let pageNumber = 1; pageNumber <= totalPages; pageNumber += 1) {
      ...
      for (const article of rows) {
        if (article.published_date < SCOPE_START) throw new Error(`Nature year filter returned an out-of-scope date: ${article.published_date}`);
    ...
    if (!known.size && articles.length !== officialTotal) {
      throw new Error(`Nature baseline validation failed: expected ${officialTotal}, collected ${articles.length}`);
    }
    if (articles.length < MINIMUM_BASELINE) throw new Error(`Only ${articles.length} Nature articles were collected`);
    const payload = {
      source: "Nature Research Articles · 2026 filter",
    ...
    console.log(`Saved ${articles.length} Nature articles to ${OUTPUT}`);
  } finally {
    await context.close();
  }
}
